'''
This file contains graph operations and strategies for generating multichannelnet (Differentiable Grid Search) final model using the weights found after the search. 
The output of this file are automatically saved in a folder named graph_operations_{date_time} 
For search / graph generation see MultiChannelNet in cnn/model_search.py which is used to perform the search and generate a networkx graph.
...
def gen_greedy_path(G, strategy="top_down"):
    '''
    Generates a single path in a greedy way from a networkx graph generated by MultiChannelNet
    # Arguments
        G: A networkx graph
        strategy: top_down or bottom_up 
         top_down strategy starts picking the most optimal operation from the source node of the graph i.e. the first layer and works its way
         to the bottom in a greedy manner
         bottom_up strategy starts picking the most optimal operation from the final node of the graph i.e. the last layer and works its way 
         to the top in a greedy manner
    '''
...
def generate_path(G, operations, directory):
    ...
    if 'flow_with_demand' in operations:
        ...
        print('Iterative cost search with demand', args.flow_demand, 'demand_step', args.demand_step, ':')
