...
parser.add_argument('--logger.info-freq', '-p', default=10, type=int,
                    metavar='N', help='logger.info frequency (default: 10)')
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    while input is not None:
        ...
        if args.local_rank == 0 and i % args.logger.info_freq == 0 and i > 1:
...
def validate(val_loader, model, criterion):
    ...
    while input is not None:
        ...
        if args.local_rank == 0 and i % args.logger.info_freq == 0:
...
def save_checkpoint(state, is_best, path='', filename='checkpoint.pth.tar', best_filename='model_best.pth.tar'):
    ...
    if is_best:
        shutil.copyfile(filename, os.path.join(path, best_filename))
